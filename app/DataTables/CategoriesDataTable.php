...
class CategoriesDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->setRowId('id')
            ->addColumn('status', function (Categories $category) {
                return new HtmlString('
                    <div class="form-check form-switch mb-2">
                        <input class="form-check-input change-status-input" type="checkbox"
                            data-category-id="' . $category->id . '" ' . ($category->is_active == 1 ? 'checked' : '') . '>
                    </div>
                ');
            })
            ...
            ->addColumn('created_at', function (Categories $category) {
            ...
            ->addColumn('action', function (Categories $category) {
            ...
            ->addColumn('image', function (Categories $category) {
                return '<img class="w-25 img-thumbnail imgRenderContainer" src="' . public_asset($category->image) . '" alt="' . $category->title . '">';
            })
     ...
     */
    public function query(Categories $category): QueryBuilder
    {
            ...
            ->parameters([
                ...
                'initComplete' => 'function () {
                    $(".change-status-input").change(function() {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "' . route('admin.ajax.categories.change.status') . '",
                            data: requestData,
                            dataType: "json",
